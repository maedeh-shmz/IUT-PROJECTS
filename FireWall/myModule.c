...
static int __init lkm_example_init(void) {
  sema_init(&s,1);//initialize semaphore
  printk(KERN_INFO "init function is called..hello:)\n");
...
static void __exit lkm_example_exit(void){
  ...
  unregister_chrdev(Major, DEVICE_NAME);
  printk(KERN_INFO"exit function is called. goodbye:(\n");
...
static ssize_t device_write(struct file *filp, const char *user_buffer, size_t len, loff_t *off)
{
  ...
  {
      printk(KERN_ALERT "my buffer is :%s\n", kernel_buffer);
